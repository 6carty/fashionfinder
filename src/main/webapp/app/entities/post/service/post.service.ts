...
type RestOf<T extends IPost | NewPost> = Omit<T, 'createdDate' | 'editedDate'> & {
...
export class PostService {
  ...
  protected convertDateFromClient<T extends IPost | NewPost | PartialUpdatePost>(post: T): RestOf<T> {
    return {
      ...post,
      createdDate: post.createdDate?.format(DATE_FORMAT) ?? null,
      editedDate: post.editedDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restPost: RestPost): IPost {
    return {
      ...
      editedDate: restPost.editedDate ? dayjs(restPost.editedDate) : undefined,
    };
  }

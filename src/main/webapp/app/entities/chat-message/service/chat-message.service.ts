...
type RestOf<T extends IChatMessage | NewChatMessage> = Omit<T, 'timestamp'> & {
...
export class ChatMessageService {
  ...
  protected convertDateFromClient<T extends IChatMessage | NewChatMessage | PartialUpdateChatMessage>(chatMessage: T): RestOf<T> {
    return {
      ...chatMessage,
      timestamp: chatMessage.timestamp?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restChatMessage: RestChatMessage): IChatMessage {
    return {
      ...restChatMessage,
      timestamp: restChatMessage.timestamp ? dayjs(restChatMessage.timestamp) : undefined,
    };
  }

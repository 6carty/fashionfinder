...
public class UserProfileResource {
    ...
    public ResponseEntity<UserProfile> partialUpdateUserProfile(
        ...
        Optional<UserProfile> result = userProfileRepository
            ...
            .map(existingUserProfile -> {
                ...
                if (userProfile.getProfilePictureContentType() != null) {
                    existingUserProfile.setProfilePictureContentType(userProfile.getProfilePictureContentType());
                }
                if (userProfile.getLastSeen() != null) {
                    existingUserProfile.setLastSeen(userProfile.getLastSeen());
                }
                if (userProfile.getLocation() != null) {
                    existingUserProfile.setLocation(userProfile.getLocation());
                }
                if (userProfile.getPrivacy() != null) {
                    existingUserProfile.setPrivacy(userProfile.getPrivacy());
                }
    ...
    public List<UserProfile> getAllUserProfiles(
        ...
        if ("calendar-is-null".equals(filter)) {
            log.debug("REST request to get all UserProfiles where calendar is null");
            return StreamSupport
                .stream(userProfileRepository.findAll().spliterator(), false)
                .filter(userProfile -> userProfile.getCalendar() == null)

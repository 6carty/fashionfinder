...
/**
 * REST controller for managing {@link team.bham.domain.Post}.
 */
@RestController
@RequestMapping("/api")
@Transactional
    ...
    public ResponseEntity<Post> partialUpdatePost(
        ...
        Optional<Post> result = postRepository
            ...
            .map(existingPost -> {
                ...
                if (post.getEditedDate() != null) {
                    existingPost.setEditedDate(post.getEditedDate());
                }
                if (post.getTotalLikes() != null) {
                    existingPost.setTotalLikes(post.getTotalLikes());
                }
